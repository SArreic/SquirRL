...
for i in range(len(args.plot)):
    try:
        if (args.plot[i] == "RL1policy"):
            ax.plot(x_axis_data, data[name_list.index(args.plot[i])][:n], label=label_dict[args.plot[i]],
                    marker=marker_dict[args.plot[i]], markersize=20, linestyle=':', color="black")
        else:
            ax.plot(x_axis_data, data[name_list.index(args.plot[i])][:n], label=label_dict[args.plot[i]],
                    marker=marker_dict[args.plot[i]], markersize=20, linestyle=':')
    except:
